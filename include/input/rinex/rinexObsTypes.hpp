 ...
 *
 * Only NormalObservation (flag = 0) represents a usable set of satellite
 ...
 *
 * The observations vector is indexed by column slot, where slot 0
 * corresponds to the first observation code listed in
 * RinexObsHeader::observationTypes[system], slot 1 to the second, and so on.
 *
 * Obtain a slot index at startup via
 * RinexObsHeader::getObservationSlotIndex(system, code), then access
 * measurements at run time via observations[slot] — a direct vector index
 * with no hash overhead.
 *
 * An observation is absent (blank field in RINEX) when
 * observations[slot].hasMeasurementValue() returns false.
 */
 ...
 * Sequential iteration over satellites is the primary access pattern in
...
struct RinexEpoch
{
     ...
     *   Valid only when hasReceiverClockOffset is true. */

    bool                              hasReceiverClockOffset = false;
    /*!< true if a receiver clock offset was present in the epoch header. */
 ...
 *
 * Built from the RINEX 3 header record SYS / # / OBS TYPES.
 * The vector index for each constellation is the column slot used to
 * index into SatelliteObservation::observations.
 *
 ...
 *
 * Parsed from the APPROX POSITION XYZ header record.
 ...
 *
 * Parsed from the RINEX 3 header record ANTENNA: DELTA H/E/N.
 ...
 *
 * The observationTypes field is the most critical member: it defines the
...
struct RinexObsHeader
{
     ...
     *
     * The slot index is the position of code in the
     * observationTypes[system] vector, which equals the index into
     * SatelliteObservation::observations for any satellite of that constellation.
     *
     ...
     * \return            0-based slot index, or -1 if system or code is not found.
     */
     ...
     *
     * Equivalent to observationTypes.at(system).size() but safe when the
     * constellation is absent from the file.
     *
     * \param[in] system  Constellation to query.
     * \return            Number of observation codes, or 0 if system is not present.
     */
 ...
 *
 * The epochs vector is in chronological order matching the file.
...
struct RinexObsData
{
     ...
     *
     * Counts every SatelliteObservation entry summed over all epochs.
